...
const Services = () => {
  return (
    <section id="services" className="mt-24">
      ...
      <div className="container m-auto px-4 grid grid-cols-1 md:grid-cols-3 space-y-2 md:space-y-0">
        {/* Map over slideData to create SwiperSlide components */}
        {slideData.map((slide) => (
          <div
            key={slide.id}
            className="w-full md:w-full h-[420px] mx-auto flex flex-col items-center justify-center"
          >
            <div className="flex items-center justify-center relative ">
              <a
                href={slide.link}
                className="hover:scale-95 transition-all duration-700 ease-in-out"
              >
                <img
                  className="w-full h-[320px] md:w-[340px] object-cover rounded-2x opacity-95 rounded-t-2xl"
                  src={slide.image}
                  alt={slide.title}
                />
                <div className="relative bottom-1 w-full md:w-[340px] bg-primary p-2 rounded-b-2xl">
